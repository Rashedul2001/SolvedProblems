...
void solve(int testCase)
{
    ...
    {
        if (res*2 >it){
            ans = max(ans, res * 2 + it);
        }
    }
    for(int i = 1; i < (int)odd.size(); i++)
    {
        if (res * 2 + odd[i-1] > odd[i])
        {
            ans = max(ans, res * 2 + odd[i-1] + odd[i]);
        }
    }
    for (auto it : even)
    {
...
{
#ifdef RASHEDUL
    freopen("D:\\SolvedProblems\\Error.txt", "w", stderr);
    freopen("D:\\SolvedProblems\\in.txt", "r", stdin);
    freopen("D:\\SolvedProblems\\out.txt", "w", stdout);
